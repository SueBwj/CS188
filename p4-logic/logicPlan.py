...
def sentence1() -> Expr:
    ...
    B = Expr('B')
    C = Expr('C')
    # 先分别对每一个expression化简然后在conjoin
    a_or_b = disjoin(A, B)
    expr2 = ~A % disjoin(~B, C)
    expr3 = disjoin(~A, ~B, C)
    sentence1 = [a_or_b, expr2, expr3]
    return conjoin(sentence1)
    "*** END YOUR CODE HERE ***"
...
def sentence2() -> Expr:
    ...
    expr3 = disjoin(~(B & ~C) >> A)
    expr4 = disjoin(~D >> C)
    sentence2 = [expr1, expr2, expr3, expr4]
    return conjoin(sentence2)
    "*** END YOUR CODE HERE ***"
...
def sentence3() -> Expr:
    ...
    expr1 = disjoin(PacmanAlive_1 % (PacmanAlive_0 &
                                     ~PacmanKilled_0 | ~PacmanAlive_0 & PacmanBorn_0))
    expr2 = disjoin(~(PacmanAlive_0 & PacmanBorn_0))
    expr3 = disjoin(PacmanBorn_0)
    sentence3 = [expr1, expr2, expr3]
    return conjoin(sentence3)
    "*** END YOUR CODE HERE ***"
...
def findModelUnderstandingCheck() -> Dict[Expr, bool]:
    ...
    a.__dict__['op'] = 'a'
    # Expr检查了初始化时，op的值一定要是大写，但是可以直接通过__dict__函数修改
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    # P => C 可以推导出 ~P | C， 但是反过来并不行
    # P => C => ~P | C 的逆否命题 p & ~C => ~(P => C)
    # 这道题目用逆否命题求解
