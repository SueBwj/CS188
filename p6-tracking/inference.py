...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict[PAC] = [(x, y) for x in
                                range(X_RANGE) for y in range(Y_RANGE)]
    variableDomainsDict[GHOST0] = [(x, y) for x in
                                   range(X_RANGE) for y in range(Y_RANGE)]
    variableDomainsDict[GHOST1] = [(x, y) for x in
                                   range(X_RANGE) for y in range(Y_RANGE)]
    # 加上正噪声
    dis_obs0 = [manhattanDistance(
        pacman_pos, ghost_pos) + MAX_NOISE for pacman_pos in variableDomainsDict[PAC] for ghost_pos in variableDomainsDict[GHOST0]]
    # 加上负噪声
    dis_obs0 += [distance for distance in range(MAX_NOISE)]
    # 加上正噪声
    dis_obs1 = [manhattanDistance(
        pacman_pos, ghost_pos) + MAX_NOISE for pacman_pos in variableDomainsDict[PAC] for ghost_pos in variableDomainsDict[GHOST1]]
    # 加上负噪声
    dis_obs1 += [distance for distance in range(MAX_NOISE)]
    variableDomainsDict[OBS0] = set(dis_obs0)
    variableDomainsDict[OBS1] = set(dis_obs1)
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        F = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        # for factor in F:
        #     print(factor)
        
        for variable in eliminationOrder:
            # 进行join操作
            F, joinedFactor = joinFactorsByVariable(F, variable)
            F.append(joinedFactor)  
            newF = []
            
            for factor in F:
                # 进行sum-out操作
                
                # 对含有variable的factor进行eliminate操作
                if variable in factor.unconditionedVariables():
                    if len(factor.unconditionedVariables()) > 1:
                        newFactor = eliminate(factor, variable)
                        newF.append(newFactor)
                    # 根据docstring的hints,如果unconditionedVariable只有一个并且是要eliminate的，或者与queryVariables无关，就可以直接去掉
                    elif len(factor.unconditionedVariables()) == 1 and not factor.unconditionedVariables().issubset(queryVariables):
                        continue
                else:
                    # 将不需要eliminate的factor也加入
                    newF.append(factor)
